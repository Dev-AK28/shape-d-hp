...
export const topHero = {
  ...
  theory: {
     ...
     * dimAt は減光開始を 0.2 に前倒し（Issue #371）: 参照HTML通りの 0.6 だと、円の収束
     * トゥイーン（position 0 duration 0.5 → t=0.5 で収束完了）と減光開始の間に無演出の間隙
     * （t=0.5〜0.6）が生じ、この間は円ラベルが完全不透明のまま中央付近で交差し、
     * 「いまの姿」「こうありたい姿」が可読な文字が重なった状態で静止して見えるスクロール
     * 幅ができてしまう。dimAt を 0.2 まで前倒しし、収束が進む間から徐々に減光することで、
     * 交差／収束のタイミングで既に減光が進行中になり、完全不透明のまま重なる区間をなくす。
     */
    borderAt: 0.5,
    labelAt: 0.72,
    dimAt: 0.2,
