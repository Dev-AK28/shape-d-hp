...
- **モバイルの縦ブレ対策（`syncTouch: true`・#444）**: `getScrollProfile()` は top/sub 両プロファイルで Lenis の `syncTouch: true` を設定する。#425（iPhone実機での縦ブレ報告）→ #426（`100vh` を `100svh` に統一）だけでは解消せず、オーナー確認により本番反映後も再現した。原因は Lenis のデフォルト（`syncTouch: false`）ではタッチ入力も wheel と同じ duration/easing ベースの仮想慣性で駆動され、iOS Safari 自身のネイティブタッチスクロール（ラバーバンド/慣性）と二重に駆動し合っていたこと。`syncTouch: true` はタッチ入力を指の動きに1:1追従させ、ネイティブスクロールとの競合を解消する。マウス/トラックパッドの wheel 入力（デスクトップの duration/easing ベース慣性）には影響しない。Lenis 自体はモバイルでも無効化しない（`shouldDisableSmoothScroll` は `prefers-reduced-motion` のみで判定・`tests/scroll/mobile-reduced-motion-policy.test.ts` で監査）方針を維持したまま対応。回帰テスト: `tests/scroll/lenis-config.test.ts`。
...
npm run build
npm run test:e2e
npm run lighthouse:check   # サーバー起動後。既定の計測対象は /services（下層）
```

- E2E: 下層ページのセクションリビール / reduced-motion は `e2e/scroll-animation.spec.ts`、トップページの演出は `e2e/top-*.spec.ts`
- Lighthouse: `scripts/lighthouse-check.mjs` + CI `lighthouse` job
  - **性能ゲート（Performance ≥ 0.9・#326）の対象は `/services`（下層ページ）**。トップ（`/`）は 10 秒のローディング演出そのものが FCP/LCP になるため対象外で、極端な劣化のみを検知する参考値（閾値 0.4）として別途計測する（#420 / 詳細は [top-particle-loader.md](./top-particle-loader.md)）
