...
- 回帰防止: `e2e/mobile-pages.spec.ts` の `expectPainted()` が **375px / 390px の両方** で対象 7 ルートについて、スクロールリビールが正常に動作することを検証する。#180 以降は `scrollIntoViewIfNeeded()` 後に `expectPainted(el, 2500)` パターンを使用（Playwright `toBeVisible()` は opacity を無視するため別途検証が必須）。デスクトップ → モバイルリサイズ後の `opacity:1` 維持は `desktop → mobile resize` describe ブロックで検証（スクロール reveal: #155 / above-fold 維持: #182）。
...
- 背景: `HomePageShell` + `CosmicScene`（fixed `z-0`、`public/hero-cosmic-bg*.webp` + `hero-nebula-layer.webp`）。`isReady` 後に `CosmicScene` をマウントしモバイル初回ハイドレーションの背景誤読込を防止。ページスクロール全体で `scale` / ネビュラ `y`+`opacity` を GSAP scrub。**Cosmic grade（#102 / #224 / #227）**: nebula 上に `.cosmic-grade-overlay`（常時・クール宇宙トーン）+ デスクトップのみ nebula filter — 詳細は [`design-system.md`](./design-system.md) の Cosmic Grade 節
...
- **ビッグバン導入演出（第1幕・#211）**: `LogoParticleFormation`（Canvas2D・hero セクション全面を覆う `absolute inset-0` オーバーレイ、`data-testid="hero-bigbang-canvas"`）が **BigBang → Drift → Gather → Logo** の連続フライト（Catmull-Rom スプライン）で粒子を中心から爆発させ、漂わせ、`shape-d-logo-transparent.png` のアルファシルエットへ収束させる。加算合成（`globalCompositeOperation: 'lighter'`）で 3 枚の生成テクスチャ（`bigbang-core/rays/nebula.webp`）＋露出ホワイトアウト・HDR ブルーム・衝撃波・星雲 3 層パララックス＋微細モート＋大気塵を重ねる。収束完了で `BrandLogo`（同一 PNG）が crossfade（Canvas は `revealMs` かけて減光 → 親が retire してアンマウント）。完了後は既存の深度通過（第2幕）へスクロールで接続。**背景シームレス露出（#224）**: Canvas は BIG BANG/DRIFT 中は `rgba(3,2,9)` の暗い「宇宙の闇」バックドロップ（α≈0.94）で背後の fixed `CosmicScene`（シルバー宇宙背景）と粒子バンドを隠し「闇から光が生まれる」状態を作る。GATHER フェーズで `easeInOutCubic` により α→0 へ後退させ、ロゴが収束するのと同調してシルバー宇宙背景が滑らかに現れる（ビッグバンの白銀トーン → シルバー背景が段差なく接続）
- パラメータ SSOT: `HERO_BIGBANG`（`lib/scroll/animation-tokens.ts`）— フェーズ時間（bigBang `1100` / drift `1200` / gather `1800` / reveal `700` ms）、品質別の粒子・塵・モート数・DPR 上限・使用テクスチャ
- 整列・スケール: Canvas は全面オーバーレイのため `logoBoxRef`（`hero-logo-stage`）の矩形から中心座標と `scale = stageWidth / sampleWidth × LOGO_PARTICLE_RENDER_SCALE`（`0.98`）を毎フレーム算出し、収束クラスタを crisp ロゴ寸法に一致させて pop を防ぐ
- 端末別フォールバック: `prefers-reduced-motion` / 低性能（`shouldDisableGsapAnimation`）時は粒子スキップ（`skipFormation`）で `BrandLogo` 即時表示。低性能・タッチ端末（`isTouchInputDevice`）は `lowPower` プロファイル（粒子数削減・塵/モート無効・星雲テクスチャ省略・DPR 上限 `1.5`）
- 粒子サンプリング: PNG を最長辺 `768px`（`LOGO_SAMPLE_MAX_DIMENSION`）にダウンサンプルして `getImageData` メモリを抑制。`HERO_BIGBANG.quality.*.sampleStep` / `particleCount` でサンプリング密度を制御。画像ロード失敗時は粒子をスキップし `BrandLogo` を表示（`onComplete` フォールバック）
- 粒子形成時間: `LOGO_PARTICLE_FORMATION_MS`（= `HERO_BIGBANG` の bigBang+drift+gather = `4100` ms）— `LogoParticleFormation` と E2E 待機の SSOT
...
| SHOWCASE | `components/home/ShowcaseSection.tsx` | **デスクトップ**: ScrollTrigger ピン留め + X方向スクラブ（4カード横並び、#215） / **モバイル**: 縦積み `getScrollRevealProps` スタガーフェードリビール（`delay: i * 0.12`）| `aria-label="サービス紹介"`。パラメータ SSOT: `SHOWCASE_HORIZONTAL`（`scrub: 1.8`、`panDuration: 1`）。コンテンツは Services ページと共通（AIプロダクト開発 / 業務自動化・DX / Webアプリ開発 / 自己表現コーチング）。`reduced-motion` / タッチデバイスは GSAP ピンなしで縦積み静的表示。**横スクロール時は stagger 無効**（`delay: 0`）— カードが 1 枚ずつ中央に入るため遅延すると空パネルが点滅するのを防ぐ。**キーボードフォーカス追従**（#247）: `useHorizontalFocusSync` フックにより `focusin` 発火時に `st.start + panelIndex * innerWidth` の Y スクロール位置へ `window.scrollTo`（instant、`behavior` 省略）で同期。**CTA アンカー遷移**（#248）: 各カードの CTA は `/services#${service.id}` を指す。`ServicesContent` の各サービスカードに `id={service.id}` を付与し、Next.js `<Link>` がハッシュ遷移で対応セクションまでスクロールする |
...
- **タイムライン進捗（全体 1s）**:
  ...
  | `historyRevealAt=0.42` | 経歴キャプション+items スタガー | 0.42 | `ABOUT_PIN_SCROLL.historyItemDuration × n + ABOUT_PIN_SCROLL.historyStagger × (n-1)` |
...
- 見出し・ピラー 1/2: `start: 'top 75%'` / `toggleActions: 'play none none reverse'` / stagger `REVEAL_OFFSET.stagger`（0.15s）
...
- E2E: `e2e/scroll-animation.spec.ts`（セクションリビール / reduced-motion）
  - `ShowcaseSection — smoke (#215)`: reduced-motion / モバイル縦積みパスのみ検証
  - `ShowcaseSection desktop horizontal scroll (#249)`: デスクトップ GSAP pin+scrub `enableHorizontal` 実挙動を検証（後述）
...
| 観点 | 判定 | 理由 |
|------|------|------|
| 宇宙ネビュラ背景と「技術の余白に創造性を」の整合 | **OK** | `#0a0a0a` ダーク背景の静けさを維持しながら **Cosmic Grade オーバーレイ**（`cosmicGrade.overlayStart`=`rgba(150,170,210,0.08)` 〜 `overlayEnd`=`rgba(150,170,210,0.15)`、`lib/design/tokens.ts`）でクール宇宙トーンの depth を上乗せ。"余白" の広がりをコスミックビジュアルで体現しており整合している。Cosmic Grade 輝度は過剰でなく、nebula のスクロール連動 opacity（`HomePageShell` で別途 scrub）とは独立して制御されている |
...
### ShowcaseSection desktop horizontal scroll

`e2e/scroll-animation.spec.ts` の `ShowcaseSection desktop horizontal scroll (#249)` テストスイート。

PR #244 レビューで指摘された「デスクトップ GSAP `enableHorizontal` 経路の未検証」を解消する。
`e2e/philosophy.spec.ts`（#184 / #239）の `[tx, iw]` co-fetch パターンを踏襲。

#### 対象コンポーネント

- `components/home/ShowcaseSection.tsx`
- `lib/scroll/animation-tokens.ts` — `SHOWCASE_HORIZONTAL`（`scrub: 1.8`）
- `lib/data/services.ts` — `SERVICE_LIST`（4カード）

#### テスト一覧

| テスト名 | 検証内容 |
|---------|---------|
| `1スクロールで第2カードへ進む` | `window.scrollBy(0, innerWidth)` 後に `panelsRef` の `translateX < -(iw × 0.8)` を確認。GSAP scrub が実際に動作していることを検証 |
| `3スクロールで全4カードを巡回できる` | 3回スクロール後に各ステップで累積進捗 `tx < -(iw × i × 0.8)` を確認。GSAP フリーズ・scrub 回帰を最短で検出 |
