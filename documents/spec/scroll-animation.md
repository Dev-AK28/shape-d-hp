...
| タッチデバイス Hero レイアウト（`prefers-reduced-motion` 時 静的フォールバック） | **OK** | `prefers-reduced-motion` 時の静的フォールバック（`flex-col h-auto` + `mt-[var(--space-6)]` CTA フロー内配置、セーフエリア対応）が適切に実装済み。対象は `isTouchInputDevice` = `isMobile \|\| prefersCoarsePointer`（#136）。通常モバイルおよび大画面タッチ端末（iPad Pro 等）でも GSAP 無効時は同フロー適用。通常スクロール有効端末では GSAP pin が有効（#137 対応）。 |
...
| **Should** ✅ | 粒子形成中（2400ms 以内）スクロール時のコピー早期出現 | [#135](https://github.com/Dev-AK28/shape-d-hp/issues/135) で設計確認済み。**B案採用**：形成中スクロールを受け入れ（ブロックしない）、`scrollRevealed` ガードでインジケーター二重表示を防止。`formationComplete=true` 時に `data-testid="hero-formation-complete"` センチネルをマウント。E2E 回帰テスト（#140）で検証済み。Lenis stop/start（A案）は没入感低下・アーキテクチャ複雑化のリスクがあり採用しない。 |
