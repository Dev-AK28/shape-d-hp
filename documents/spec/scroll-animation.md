...
- 背景: `HomePageShell` + `CosmicScene`（fixed `z-0`、`public/hero-cosmic-bg*.webp` + `hero-nebula-layer.webp`）。`isReady` 後に `CosmicScene` をマウントしモバイル初回ハイドレーションの背景誤読込を防止。ページスクロール全体で `scale` / ネビュラ `y`+`opacity` を GSAP scrub。**Cosmic grade（#102 / #224）**: nebula 上に `.cosmic-warm-grade-overlay`（常時・クール宇宙トーン）+ デスクトップのみ nebula filter — 詳細は [`design-system.md`](./design-system.md) の Cosmic Grade 節
