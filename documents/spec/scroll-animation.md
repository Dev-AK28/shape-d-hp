...
- **ビッグバン導入演出（第1幕・#211）**: `LogoParticleFormation`（Canvas2D・hero セクション全面を覆う `absolute inset-0` オーバーレイ、`data-testid="hero-bigbang-canvas"`）が **BigBang → Drift → Gather → Logo** の連続フライト（Catmull-Rom スプライン）で粒子を中心から爆発させ、漂わせ、`shape-d-logo-transparent.png` のアルファシルエットへ収束させる。加算合成（`globalCompositeOperation: 'lighter'`）で 3 枚の生成テクスチャ（`bigbang-core/rays/nebula.webp`）＋露出ホワイトアウト・HDR ブルーム・衝撃波・星雲 3 層パララックス＋微細モート＋大気塵を重ねる。収束完了で `BrandLogo`（同一 PNG）が crossfade（Canvas は `revealMs` かけて減光 → 親が retire してアンマウント）。完了後は既存の深度通過（第2幕）へスクロールで接続
...
全 #210 シリーズを通じた端末別出し分けは `lib/performance/device-profile.ts` の2関数で統一されている。
詳細は [`mobile-performance.md`](./mobile-performance.md) の「全幕統一方針」節と [`performance-budget.md`](./performance-budget.md) を参照。

| 関数 | 条件 | 効果 |
|------|------|------|
| `shouldDisableGsapAnimation(profile)` | `prefersReducedMotion` のみ | 全幕の GSAP を無効化 |
| `isTouchInputDevice(profile)` | `isMobile \|\| prefersCoarsePointer` | ピン廃止 / パララックス縮小 |
...
ポリシー遵守は `tests/scroll/mobile-reduced-motion-policy.test.ts` が全幕横断で監査する（19 assertions）。
