...
| `.cosmic-nebula-layer` | `mix-blend-mode: screen` / `opacity: 0.26` | 宇宙背景と二重の warm-gold 星雲が screen で重なり四隅が白飛びするのを防ぐため opacity を抑制 |

`data-testid="cosmic-warm-grade-overlay"` で E2E 検証可能。背景 `#0a0a0a` の暗さ・ミニマルさは維持する。warm-gold AI 背景アセット（`hero-cosmic-bg*.webp`）は彩度の高い星雲を持つため、ミニマルさ維持の目的で輝度を約 50% に抑えて書き出している。

### アセットファイルサイズ（参考）

| アセット | 変更前 | 変更後 | 備考 |
|---------|--------|--------|------|
| `hero-cosmic-bg.webp`（desktop） | 124 KB | 67 KB | 2026-06-21 輝度約 50% 減光 |
| `hero-cosmic-bg-mobile.webp`（mobile） | 139 KB | 77 KB | 2026-06-21 輝度約 50% 減光 |
