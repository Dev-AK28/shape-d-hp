...
| ホームセクション共有 | `lib/design/section-typography-classes.ts` | 旧ホームセクション（About / MissionVision）の見出し・caption 用。consumers は #302 で撤去済みで、現在は `tests/design/tokens.test.ts` のみが参照（レガシー・別途コード整理の対象） |
...
- `lib/design/section-typography-classes.ts` — 旧ホーム About / MissionVision 共有 class 文字列（consumers 撤去済み・レガシー）
