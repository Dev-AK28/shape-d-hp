...
| `.cosmic-nebula-layer` | `mix-blend-mode: screen` / `opacity: 0.42` | warm-gold は CSS グレード overlay で一元管理。素材画像は色味を焼き込まない素直な版を使用する（Issue #202 で AI 焼き込み版から復元） |

`data-testid="cosmic-warm-grade-overlay"` で E2E 検証可能。背景 `#0a0a0a` の暗さ・ミニマルさは維持する。warm-gold の温度感は素材画像ではなく CSS グレード overlay（`.cosmic-warm-grade-overlay` / `--warm-grade-nebula-filter`）で付与する設計とし、色味調整を1箇所に集約してテイストのブレを防ぐ。
...
| `hero-nebula-layer.webp` | パララックス前景（`mix-blend-mode: screen`、透過 WebP） |
