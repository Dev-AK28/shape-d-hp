...
`/services`・`/works`・`/process`・`/contact` で共通の `components/ui/PageHeader.tsx` を使用する。
...
| 背景 | 通常は gradient のみ。`/process` は `starBackground` で `StarBackground` を重ねる |
| テスト | `data-testid="page-header"` |
