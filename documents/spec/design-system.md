...
> 旧セクション（About / MissionVision / ShowcaseSection）は参照側（#305〜#310）で置換済みだが、既存の回帰テスト（#149/#159/#180/#215/#249 等）が結びついているため #311 時点では据え置き。`HomePageShell`（cosmic 背景）ごとの撤去は #312（cosmic/velocity-skew 無効化）と #316 のクリーンアップで実施する。
