...
- **タイムラインの起点（3 分岐・PR #419 レビュー対応）**: `originMs` が決める。framer の delay / duration もシェーダのクロックも同じ起点を共有する
  ...
  - **実ロゴは「その時刻にあるべき不透明度」から立ち上げる**（Issue #421）。粒子はシェーダの絶対時刻で `1 - uHandoff` に減衰するため、ハイドレーションが handoff の途中（8.0〜9.0 秒）に食い込むと、マウント時点で粒子は既に半分消えている。実ロゴを常に 0 から立ち上げると、その差分だけ**「粒子も実ロゴも薄い」数百 ms の谷**ができる。開始値は `handoffRevealOpacity()`（`lib/loader/particle-logo.ts` — シェーダの `uHandoff` と同じ Hermite 曲線）で求め、**粒子の減衰分をちょうど埋める**（合計が常に 1 になる）
    - ⚠️ **開始値の補正は必ず effect（ハイドレーション後）で `useAnimationControls` の `set()` から行うこと**。framer の `initial` prop に入れると **SSR HTML にも出力される**ため、サーバー（クロックが無いので 0）とクライアント（補正値）が食い違い、ハイドレーション不一致になる
...
- **Given** 低速回線でトップページを開き、ハイドレーションが handoff の途中（8.0〜9.0 秒）に完了する（#421）
- **When** 実ロゴが立ち上がり始める
- **Then** 実ロゴは 0 からではなく「その時刻にあるべき不透明度」から立ち上がり、粒子の減衰分をちょうど埋める（**粒子も実ロゴも薄い区間が生じない**）
- **かつ** SSR とクライアントの初期描画が一致し、ハイドレーション不一致の警告が出ない
