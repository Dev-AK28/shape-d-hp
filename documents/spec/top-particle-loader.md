...
| フェーズ | 長さ | 内容 |
|---------|------|------|
| drift | 2500ms | 粒子が散開位置で浮遊（まだ収束しない）。実ロゴはゴースト表示（`LOGO_GHOST_OPACITY` = 0.16） |
| converge | 4000ms | 波状スタッガーで緩やかに収束（進捗 55% まで） |
| snap | 1500ms | 一気に加速してロゴへ吸着（残り 45%） |
| **handoff（#418）** | **1000ms** | **粒子（`uHandoff` で alpha 減衰）が薄れ、実ロゴ `<img>` が opacity 0.16 → 1 へ立ち上がる** |
| hold | 300ms | 実ロゴを見せる |
...
- ~~**粒子のフェードインを長くする**~~ → #414 時点では FCP を粒子に頼っていたため禁止事項だったが、**#418 で FCP/LCP は実ロゴ `<img>` が担うようになり、かつ #419 でクロックがナビゲーション起点になった**（three.js のロード完了時点で `uTime > 250ms` のため `fadeIn` は初回フレームから 1）。シェーダの `fadeIn` は現在ほぼ機能しておらず、Lighthouse への影響もない
...
- **キーボード操作で即スキップ**（WCAG 2.4.7 の緩和・PR #419 レビュー対応）: 不透明オーバーレイ + `pointer-events-none` のため、演出中はヒーローのリンクにフォーカスが通るのにフォーカスリングが見えない。`keydown` を検知したら演出を即座に終了してページ本体を見せる
- **残るトレードオフ**: マウスクリックは演出をスキップしない（クリックは背後の要素に通る）。「操作をブロックしない」ことを優先した意図的な選択
