...
- **And** これは framer-motion リビール消費コンポーネントが `useStaticReveal()` 経由で `staticReveal` を取得し渡すことで保証される（`TextReveal` は初回 `staticReveal` をラッチして hydration 後も即時表示を維持）。詳細: [`scroll-animation.md`](./scroll-animation.md) の staticReveal ガード節
