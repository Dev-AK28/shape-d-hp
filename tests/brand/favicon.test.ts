...
describe('committed favicon assets', () => {
  it.each(FAVICON_OUTPUTS)(
    'matches freshly generated bytes for $filename',
    async ({ size, filename }) => {
      const appDir = join(process.cwd(), 'app');
      const committed = await readFile(join(appDir, filename));
      const generated = await renderFavicon(size);
      ...
      expect(generatedHash).toBe(committedHash);
    },
  );
});
