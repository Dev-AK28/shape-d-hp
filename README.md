...
| 送信元メール | 設定済み | `RESEND_FROM_EMAIL=onboarding@resend.dev`（ドメイン検証前の暫定。検証完了までは `hello@shape-d.com` 宛送信が 403 になる） |
| Resend ドメイン検証 | **要対応**（[#69](https://github.com/Dev-AK28/shape-d-hp/issues/69)） | 手順は下記「Resend ドメイン検証手順」を参照。ダッシュボード / DNS / Vercel 管理画面での手動操作が必須のため人手対応が必要 |

#### Resend ドメイン検証手順（[#69](https://github.com/Dev-AK28/shape-d-hp/issues/69)）

1. [resend.com/domains](https://resend.com/domains) で `shape-d.com` を追加する
2. 発行された DNS レコード（SPF / DKIM 等）をドメイン管理画面（DNS プロバイダ）に設定する
3. Resend ダッシュボードでドメインが **Verified** になったことを確認する
4. Vercel Production の環境変数 `RESEND_FROM_EMAIL` を `hello@shape-d.com`（または検証済みの送信元アドレス）に更新する
5. Production を再デプロイし、お問い合わせフォームから実際にテスト送信して届くことを確認する

> Resend の Send-only API キーではドメイン追加 API は利用不可。ダッシュボードからドメイン検証を行ってください。検証完了までは `hello@shape-d.com` 宛送信が 403 になります。上記 1〜5 はいずれも外部ダッシュボード / DNS / Vercel 管理画面での手動操作であり、リポジトリ内のコード変更だけでは完了しません。
